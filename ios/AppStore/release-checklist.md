# Maker Business Lab iOS 1.0 Release Checklist

- [x] Native SwiftUI navigation shell with live website content and offline cache fallback
- [x] English default and Simplified Chinese in-app language switch
- [x] Five native tabs: Home, Opportunities, Calculate, Radar, Learn
- [x] H5 content revalidates on launch, tab activation, foreground return, and pull-to-refresh
- [x] Build 13 adds the brighter red-and-gold profit icon and compact native header
- [x] Live first-party H5 content with last-rendered-page fallback
- [x] Local language preference
- [x] Transparent calculation boundaries and no income promises
- [x] GPT Image 2 app icon and Radar hero asset
- [x] Unit tests and simulator launch
- [x] Bilingual support and privacy URLs prepared
- [ ] Large and small iPhone visual QA
- [ ] Release archive signed with Apple Distribution
- [ ] App Store Connect app record and processed build
- [ ] English and Chinese metadata/screenshots uploaded
- [ ] App privacy, age rating, export compliance, and global availability completed
- [ ] Submitted for review
